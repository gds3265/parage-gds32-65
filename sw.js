...
const CORE=[
  './index.html','./index.html?v=4.0.24','./style.css?v=4.0.24','./app.js?v=4.0.24',
  './manifest.json?v=4.0.24','./version.json','./app-logo.png?v=4.0.24',
  './icon-192.png?v=4.0.24','./icon-512.png?v=4.0.24','./apple-touch-icon.png?v=4.0.24','./favicon.png?v=4.0.24'
];
  ...
  if(event.request.mode==='navigate'||url.pathname.endsWith('/index.html')||url.pathname.endsWith('/version.json')){
    event.respondWith((async()=>{
      try{
        ...
        return (await caches.match(event.request)) || (await caches.match('./index.html')) || (await caches.match('./index.html?v=4.0.24')) || Response.error();
      }
    })());
    return;
  }
