...
const CORE=[
  './index.html','./index.html?v=4.0.21','./style.css?v=4.0.21','./app.js?v=4.0.21',
  './manifest.json?v=4.0.21','./version.json','./app-logo.png?v=4.0.21',
  './icon-192.png?v=4.0.21','./icon-512.png?v=4.0.21','./apple-touch-icon.png?v=4.0.21','./favicon.png?v=4.0.21'
];
  ...
  if(event.request.mode==='navigate'||url.pathname.endsWith('/index.html')||url.pathname.endsWith('/version.json')){
    event.respondWith((async()=>{
      try{
        ...
        return (await caches.match(event.request)) || (await caches.match('./index.html')) || (await caches.match('./index.html?v=4.0.21')) || Response.error();
      }
    })());
    return;
  }
